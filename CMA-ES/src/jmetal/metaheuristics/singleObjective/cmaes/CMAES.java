...
public class CMAES extends Algorithm {
	...
	private boolean isFeasible(Solution solution) throws JMException {
		...
		for (int i = 0; i < problem_.getNumberOfVariables(); i++) {
			float value = (float) x[i].getValue();
	...
	private void updateDistribution() throws JMException {
		...
		for (int i = 0; i < N; i++) {
			artmp[i] = 0;
			//float value = (xmean[i] - xold[i]) / sigma;
			for (int j = 0; j < N; j++) {
		...
		for (int i = 0; i < N; i++) {
			ps[i] = (float) ((1. - cs) * ps[i]
					+ Math.sqrt(cs * (2. - cs) * mueff)
					* artmp[i]);
		}

		// calculate norm(ps)^2
		float psxps = (float) 0.0;
		for (int i = 0; i < N; i++) {
			psxps += ps[i] * ps[i];
		}
		...
		if (counteval - eigeneval > lambda /(c1+cmu)/N/10) { // to achieve O(N^2)
			...
			for (int i = 0; i < N; i++) {
				...
				diagD[i] = (float) Math.sqrt(diagD[i]);
			}
